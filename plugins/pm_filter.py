    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BΔCҜ", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"📚 PΔGΣ:{math.ceil(int(offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"📆 {math.ceil(int(offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons"),
             InlineKeyboardButton("ПΣXT ⏭️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("⏪ BΔCҜ", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings['shortlink']:
        btn.insert(0,
            [InlineKeyboardButton("✨ ɢᴇᴛ ᴀʟʟ ғɪʟᴇs ✨", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{query.message.chat.id}_{key}'))]
        )
    else:
        btn.insert(0,
            [InlineKeyboardButton("✨ ɢᴇᴛ ᴀʟʟ ғɪʟᴇs ✨", callback_data=f"send_all#{key}")]
        )
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BΔCҜ", callback_data=f"lang_next#{req}#{key}#{lang}#{b_offset}#{offset}"),
             InlineKeyboardButton(f"📚 PΔGΣ:{math.ceil(int(l_offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons")]
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        if settings['shortlink']:
            btn.insert(0,
                [InlineKeyboardButton("✨ ɢᴇᴛ ᴀʟʟ ғɪʟᴇs ✨", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{message.chat.id}_{key}'))]
            )
        else:
            btn.insert(0,
                [InlineKeyboardButton("✨ ɢᴇᴛ ᴀʟʟ ғɪʟᴇs ✨", callback_data=f"send_all#{key}")]
